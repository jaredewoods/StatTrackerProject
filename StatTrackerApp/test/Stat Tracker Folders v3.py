...
def start_new_game_log():
    ...
    game_info = {}  # Create a dictionary to store game info

    try:
        game_info["date"] = date_entry.get().strip()
        game_info["date"] = datetime.datetime.strptime(
            game_info["date"], "%m/%d/%y"
        ).strftime("%m.%d.%y")

        game_info["start_time"] = start_time_entry.get().strip()
        start_time_obj = datetime.datetime.strptime(game_info["start_time"], "%H:%M")
        game_info["start_time"] = start_time_obj.strftime("%I:%M%p")
    except ValueError as e:
        messagebox.showerror(
            "Invalid Date or Time", "Please enter a valid date and time."
        )
    ...
    game_info["quarter"] = f"{game_info['quarter']}"
...
def add_event(video_time, player_number, event_code):
    try:
        # Ensure player_number is an integer and exists in the team_roster
        player_number = int(player_number)
        if player_number not in team_roster:
            raise ValueError("Invalid player number.")

        player_name = f"{team_roster[player_number][0]} {team_roster[player_number][1]}"

        # Check if event_code is valid
        ...
        event_data = [
            ...
            video_time,
            str(player_number),
            team_roster[player_number][0],
            team_roster[player_number][1],
            event_description,
            event_code,
        ]
...
def export_game_data_to_excel():
    try:
        ...
        # Save the workbook with a new name in the 'output' folder
        ...
        # Attempt to open the newly created Excel file
        subprocess.Popen(["open", excel_filename])

    except FileNotFoundError:
        export_status_label.config(text="Error: Excel template file not found.")
    except PermissionError:
        export_status_label.config(text="Error: Permission denied for file operations.")
    except subprocess.CalledProcessError:
        export_status_label.config(text="Error: Unable to open the Excel file.")
    except Exception as e:
        export_status_label.config(text=f"Error: {str(e)}")
...
def update_timecode():
    """Update the timecode label in the Tkinter window."""
    while True:
        try:
            ...
            root.after(0, lambda: timecode_label.config(text=formatted_timecode))

        except ValueError as ve:
            root.after(0, lambda: timecode_label.config(text="Invalid Timecode"))
            print(f"Value Error: {ve}")
        except Exception as e:
            root.after(0, lambda: timecode_label.config(text="Error"))
            print(f"Error: {e}")

        time.sleep(0.05)  # Update twenty times a second
...
def open_calendar(
    event, entry_widget
):  # Displays calendar for date selection in entry.
    if not hasattr(open_calendar, "is_open") or not open_calendar.is_open:
        ...
        tk.Button(top, text="OK", command=on_select_date).pack()


def confirm_quit():  # Prompts confirmation before closing application.
...
threading.Thread(target=update_timecode, daemon=True).start()
