...
# Create a button with an image ('edit_icon.png')
image_path = "../resources/edit_icon.png"

# Load the image using PIL and convert it to a PhotoImage object
icon_image = Image.open(image_path)
icon_image = ImageTk.PhotoImage(icon_image)

# Create the button with the icon image
open_editor_button = ttk.Button(frame_center, image=icon_image, command=open_team_roster_editor)
open_editor_button.grid(column=0, row=1, padx=0, pady=10, sticky="e")

# Keep a reference to the icon image to prevent it from being garbage collected
open_editor_button.icon_image = icon_image
